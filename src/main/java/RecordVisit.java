...
public class RecordVisit extends Sequence {
    ...
    public void msg1() throws ParseException {
        Pattern pattern = Pattern.compile("^(0[1-9]|1[012])\\/(0[1-9]|[12][0-9]|3[01])\\/(19|20)\\d\\d\\s([0-1][1-9]|2[0-3]):([0-5][0-9])$");
        Matcher matcher = pattern.matcher(response);
        System.out.println("Message 2/2");

        if(matcher.matches()) {
            SimpleDateFormat df = new SimpleDateFormat("MM/dd/yyyy hh:mm");
            Date visitDate = df.parse(response);
            ...
            String prompt = "Your visit has successfully been recorded!\n\n" +
                    "Returning you to the main menu!";
            conversation.getHoneyBeeFarmer().sendSMS(prompt);
            System.out.println(prompt);
            this.endSequence();
        } else {
            String prompt = "The date is invalid. Please use the format mm/dd/yyyy hh:mm. Do not include anything else in the message.\n\nExample:\n06/22/2021 09:30";
            conversation.getHoneyBeeFarmer().sendSMS(prompt);
            System.out.println(prompt);
        }
    }
